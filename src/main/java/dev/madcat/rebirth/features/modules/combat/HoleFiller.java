...
public class HoleFiller extends Module
{
    ...
    public HoleFiller() {
        ...
        this.range = (Setting<Double>)this.register(new Setting("Range", 4.5, 0.1, 6.0));
        this.smartRange = (Setting<Double>)this.register(new Setting("HoleRange", 3.0, 0.1, 6.0));
        ...
        this.alpha = (Setting<Integer>)this.register(new Setting("Alpha", 0, 0, 255, v -> !this.rainbow.getValue()));
        this.outlineAlpha = (Setting<Integer>)this.register(new Setting("OL-Alpha", 0, 0, 255, v -> !this.rainbow.getValue()));
        this.setInstance();
    }
    ...
    public void onUpdate() {
        ...
        if (this.web.getValue()) {
            obsidianSlot = ((HoleFiller.mc.player.getHeldItemMainhand().getItem() == Item.getItemFromBlock(Blocks.WEB)) ? HoleFiller.mc.player.inventory.currentItem : -1);
            if (obsidianSlot == -1) {
                for (int l = 0; l < 9; ++l) {
                    if (HoleFiller.mc.player.inventory.getStackInSlot(l).getItem() == Item.getItemFromBlock(Blocks.WEB)) {
                        obsidianSlot = l;
                        break;
                    }
                }
        ...
        if (q != null && HoleFiller.mc.player.onGround) {
            if (BreakCheck.Instance().BrokenPos != null && new BlockPos((Vec3i)BreakCheck.Instance().BrokenPos).equals((Object)new BlockPos((Vec3i)this.render))) {
                return;
            }
    ...
    private void findClosestTarget() {
        ...
        for (final EntityPlayer target : playerList) {
            if (target != HoleFiller.mc.player && !Rebirth.friendManager.isFriend(target.getName()) && EntityUtil.isLiving((Entity)target)) {
                if (target.getHealth() <= 0.0f) {
                    continue;
                }
                if (this.closestTarget != null && HoleFiller.mc.player.getDistance((Entity)target) >= HoleFiller.mc.player.getDistance((Entity)this.closestTarget)) {
                    continue;
                }
