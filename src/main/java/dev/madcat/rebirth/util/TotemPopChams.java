...
public class TotemPopChams
{
    ...
    public void onRenderWorld(final RenderWorldLastEvent event) {
        ...
        final Color lineColorS = new Color((int)PopChams.rL.getValue(), (int)PopChams.gL.getValue(), (int)PopChams.bL.getValue(), (int)PopChams.aL.getValue());
        final Color fillColorS = new Color((int)PopChams.rF.getValue(), (int)PopChams.gF.getValue(), (int)PopChams.bF.getValue(), (int)PopChams.aF.getValue());
        int lineA = lineColorS.getAlpha();
        int fillA = fillColorS.getAlpha();
        final long time = System.currentTimeMillis() - this.startTime - (long)PopChams.fadestart.getValue();
        if (System.currentTimeMillis() - this.startTime > (long)PopChams.fadestart.getValue()) {
            double normal = this.normalize((double)time, 0.0, (double)PopChams.fadetime.getValue());
