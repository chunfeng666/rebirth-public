...
public class ConfigManager implements Util
{
    ...
    public static void setValueFromJson(final Feature feature, final Setting setting, final JsonElement element) {
        ...
        switch (type) {
            ...
            default: {
                Rebirth.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
            }
        }
    }
    ...
    private static void loadFile(final JsonObject input, final Feature feature) {
        for (final Map.Entry<String, JsonElement> entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    Rebirth.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                }
                catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public void loadConfig(final String name) {
        final List<File> files = Arrays.stream((Object[])Objects.requireNonNull((T[])new File("Rebirth").listFiles())).filter(File::isDirectory).collect((Collector<? super Object, ?, List<File>>)Collectors.toList());
    ...
    public void saveConfig(final String name) {
        ...
        Rebirth.friendManager.saveFriends();
        for (final Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        final File currentConfig = new File("Rebirth/currentconfig.txt");
        try {
    ...
    public void init() {
        ...
        this.features.add(Rebirth.friendManager);
    ...
    private void loadPath(final Path path, final Feature feature) throws IOException {
        ...
        try {
            ...
            Rebirth.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
