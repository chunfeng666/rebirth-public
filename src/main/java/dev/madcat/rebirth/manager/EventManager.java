...
public class EventManager extends Feature
{
    ...
    public void onUpdate(final LivingEvent.LivingUpdateEvent event) {
        if (!fullNullCheck() && event.getEntity().getEntityWorld().isRemote && event.getEntityLiving().equals((Object)EventManager.mc.player)) {
            Rebirth.inventoryManager.update();
            Rebirth.moduleManager.onUpdate();
            if (HUD.getInstance().renderingMode.getValue() == HUD.RenderingMode.Length) {
                Rebirth.moduleManager.sortModules(true);
            }
            else {
                Rebirth.moduleManager.sortModulesABC();
            }
        }
    }
    ...
    public void onClientConnect(final FMLNetworkEvent.ClientConnectedToServerEvent event) {
        ...
        Rebirth.moduleManager.onLogin();
    }
    
    @SubscribeEvent
    public void onClientDisconnect(final FMLNetworkEvent.ClientDisconnectionFromServerEvent event) {
        Rebirth.moduleManager.onLogout();
    }
    ...
    public void onUpdateWalkingPlayer(final UpdateWalkingPlayerEvent event) {
        ...
        if (event.getStage() == 0) {
            Rebirth.speedManager.updateValues();
            Rebirth.rotationManager.updateRotations();
            Rebirth.positionManager.updatePosition();
        }
        if (event.getStage() == 1) {
            Rebirth.rotationManager.restoreRotations();
            Rebirth.positionManager.restorePosition();
        }
    }
    ...
    public void onPacketReceive(final PacketEvent.Receive event) {
        ...
        Rebirth.serverManager.onPacketReceived();
    ...
    public void onWorldRender(final RenderWorldLastEvent event) {
        ...
        Rebirth.moduleManager.onRender3D(render3dEvent);
        GlStateManager.glLineWidth(1.0f);
    ...
    public void renderHUD(final RenderGameOverlayEvent.Post event) {
        if (event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR) {
            Rebirth.textManager.updateResolution();
        }
    }
    ...
    public void onRenderGameOverlayEvent(final RenderGameOverlayEvent.Text event) {
        if (event.getType().equals((Object)RenderGameOverlayEvent.ElementType.TEXT)) {
            ...
            Rebirth.moduleManager.onRender2D(render2DEvent);
    ...
    public void onKeyInput(final InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState()) {
            Rebirth.moduleManager.onKeyPressed(Keyboard.getEventKey());
        }
    }
        ...
        if (event.getMessage().startsWith(Command.getCommandPrefix())) {
            ...
            try {
                ...
                if (event.getMessage().length() > 1) {
                    Rebirth.commandManager.executeCommand(event.getMessage().substring(Command.getCommandPrefix().length() - 1));
                }
                else {
